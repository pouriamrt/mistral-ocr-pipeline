...
class ExtractionOutcomes(BaseModel):
    ...
    timing_of_measurement: Optional[
        ...
        description=(
            ...
            "but ONLY when the study EXPLICITLY describes the sampling protocol as one of the four allowed "
            "categories (Peak, Trough, Residual / pre-procedural, or Serial PK/PD) in its own Methods, "
            "Results, figure/table captions, or pharmacokinetic sections.\n\n"
            "═══════════════════════════════════════════════\n"
            "STRICT NULL-FOR-UNCLEAR POLICY (CRITICAL)\n"
            "═══════════════════════════════════════════════\n"
            "This field MUST be left null (do NOT populate) UNLESS the paper explicitly states the sample "
            "was timed as one of the four allowed categories. Populating any value when the paper does not "
            "explicitly describe the sampling protocol is a POSITIVE MISCLASSIFICATION and is more harmful "
            "than leaving the field blank.\n\n"
            "RETURN null WHEN:\n"
            "• Timing is described as random, opportunistic, convenience, leftover, discarded, routine-care, "
            "or 'at time of clinical visit' / 'at admission' / 'at presentation' with no dose-timing linkage.\n"
            "• Timing is reported only as a baseline characteristic such as 'median time since last dose 5 h, "
            "range 2–27 h' WITHOUT a controlled sampling design (peak/trough/residual/serial).\n"
            "• Timing windows are broad and span peak and trough (e.g., '2–27 hours after intake', '1–24 h "
            "since last dose') with no protocol designation.\n"
            "• Timing is anchored to clock time only (e.g., 'samples drawn 8–10 am') with no linkage to dose.\n"
            "• Only vague language is used (e.g., 'blood was collected', 'samples were obtained') without "
            "any timing context.\n"
            "• You cannot find an explicit Methods/Results/PK/caption sentence mapping the sample to "
            "Peak, Trough, Residual, or Serial PK/PD.\n\n"
            "DO NOT auto-classify ambiguous, uncontrolled, opportunistic, or 'random' timing into any "
            "category. Do NOT infer timing from study genre, DOAC class, manufacturer recommendations, "
            "PK patterns, or contextual hints. There is no 'random' or 'not reported' fallback — null IS "
            "the honest signal for those cases.\n\n"
            "═══════════════════════════════════════════════\n"
            ...
            "If timing information appears ONLY in figure/table captions or PK subsections, it is still valid. "
            "But if NO section contains an explicit mapping to Peak / Trough / Residual / Serial PK/PD, return null.\n\n"
            "═══════════════════════════════════════════════\n"
            ...
            "- Genre F (Perioperative): If samples are drawn after a PLANNED DOAC interruption before surgery, "
            "classify as 'Residual level after interruption / pre-procedural'. If sampling timing is not "
            "explicitly described as residual/pre-procedural, return null — do NOT default to any category.\n"
            "- Genre E (Case reports): Narrative timing descriptions are valid ONLY if they map the sample "
            "to one of the four allowed categories. Otherwise return null.\n"
            "- Genre A / urgent-care studies: If samples are drawn at presentation (stroke, hip fracture, "
            "trauma, urgent admission) and the study reports time-since-last-dose only as a baseline "
            "characteristic with no fixed peak/trough/residual protocol, return null. Do NOT assign any "
            "category by inference.\n\n"
            "═══════════════════════════════════════════════\n"
            "TIMING LEXICON (PHRASE-TO-CATEGORY MAPPING)\n"
            "═══════════════════════════════════════════════\n"
            "Use this lexicon to map phrases to the four allowed categories. If a phrase does NOT clearly "
            "map to one of these four, return null for that sample/cohort.\n\n"
            ...
            "• narrow post-dose window of <6 hours after administration AND explicitly described as peak/post-dose\n"
            ...
            "PHRASES THAT MUST RESULT IN null (NOT in any category):\n"
            "• 'random level', 'random sample', 'opportunistic sample', 'convenience sample'\n"
            "• 'routine-care sample', 'discarded sample', 'leftover sample'\n"
            "• 'at presentation', 'at admission', 'at time of event' (with no dose-timing linkage)\n"
            "• 'samples drawn at clinical visit / clinic appointment / follow-up' without dosing linkage\n"
            "• broad heterogeneous windows that span peak and trough (e.g., '2–27 hours after intake', "
            "'median 5 h, range 1–24 h since last dose')\n"
            "• clock-time-based sampling with NO linkage to last/next dose (e.g., 'samples from 8–10 am')\n"
            "• samples drawn according to immediate clinical need (emergency/urgent contexts) without "
            "an explicit residual/pre-procedural protocol\n"
            "• 'blood was collected' / 'samples were obtained' with no timing context\n"
            "• timing-since-last-dose reported as a baseline characteristic only, with no controlled "
            "sampling protocol (e.g., 'median time since last dose 5 h, range 2–27 h')\n"
            "For ALL of the above, return null. Do NOT shoehorn into Peak, Trough, Residual, or Serial PK/PD.\n\n"
            "═══════════════════════════════════════════════\n"
            "CLASSIFICATION RULES\n"
            "═══════════════════════════════════════════════\n"
            "1) Extract timing ONLY from the sections listed in SECTION ROUTING above.\n"
            "2) Apply the TIMING LEXICON. Only emit a category when its phrase maps EXPLICITLY to that category.\n"
            "3) Do NOT infer timing from DOAC class, manufacturer recommendations, or PK patterns.\n"
            "4) If a study collects samples at MULTIPLE explicit timings (e.g., both peak and trough), "
            ...
            "specifically describe one of the four timing categories for the included studies.\n"
            "7) If timing is not clearly Peak, Trough, Residual, or Serial PK/PD, return null. There is "
            "NO Random or Not-reported fallback — null is the correct output.\n\n"
            "CRITICAL DISTINCTION: RESIDUAL vs TROUGH:\n"
            ...
            "A paper that describes 'pre-procedural samples after DOAC interruption' is RESIDUAL.\n"
            "A paper that describes 'predose samples at steady state' is TROUGH.\n"
            "A paper that describes 'samples drawn at emergency presentation' with no further protocol is "
            "NEITHER — return null.\n\n"
            "COMMON ERROR MODES TO AVOID:\n"
            "• Selecting any category based on time-since-last-dose reported only as a baseline "
            "characteristic — that is NOT a sampling protocol.\n"
            "• Collapsing broad/heterogeneous timing windows into Peak or Trough — return null instead.\n"
            "• Selecting 'Serial PK/PD profile' when only peak and/or trough were measured.\n"
            "• Failing to map Cmin/Ctrough/predose terms to 'Trough'.\n"
            "• Failing to map Cmax/Tmax/post-dose terms to 'Peak'.\n"
            "• Populating a category as a 'best guess' when the paper does not clearly describe a "
            "sampling protocol — return null instead.\n\n"
            "OUTPUT:\n"
            "Return a list containing one or more of {'Peak level (2–4 hours post-dose)', "
            "'Trough level (just prior to next dose)', 'Residual level after interruption / pre-procedural', "
            "'Serial PK/PD profile'}, ONLY when each is EXPLICITLY supported by Methods / Results / PK / "
            "figure-table caption text. If no category is explicitly supported, return null. "
