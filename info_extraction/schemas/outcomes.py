...
class ExtractionOutcomes(BaseModel):
    ...
    clinical_outcomes_measured: Optional[Literal["Yes", "No"]] = Field(
        ...
        description=(
            ...
            "═══════════════════════════════════════════════\n"
            "HARD GATE (BOTH CONDITIONS MUST BE MET)\n"
            "═══════════════════════════════════════════════\n"
            "Set to 'Yes' ONLY if BOTH conditions are satisfied:\n\n"
            "Condition 1 (METHODS/ENDPOINTS): The outcome must be explicitly planned or measured as stated in "
            "Methods, Endpoints, Study Procedures, or Outcome Measures sections.\n"
            "Look for: 'we recorded', 'we assessed', 'primary endpoint', 'secondary endpoint', "
            "'outcomes included', 'bleeding events were recorded', 'follow-up for clinical events', "
            "'adjudication of outcomes'.\n\n"
            "Condition 2 (RESULTS): The outcome must be reported in the Results section for THIS study's cohort. "
            "Look for: actual event counts, event rates, explicit 'no events occurred' statements, "
            "tables/figures showing clinical events, time-to-event analyses.\n\n"
            "If EITHER condition fails, set to 'No'.\n\n"
            "═══════════════════════════════════════════════\n"
            "SECTION ROUTING (WHERE TO LOOK)\n"
            "═══════════════════════════════════════════════\n"
            "SEARCH (in order): Outcome Measures, Endpoints, Methods, Results sections.\n"
            "BACKUP: Abstract Methods and Abstract Results.\n"
            "IGNORE COMPLETELY: Introduction, Background, Discussion, References.\n"
            "Outcomes mentioned ONLY in Introduction or Discussion are NOT study outcomes. "
            "They are background context and MUST NOT be extracted.\n\n"
            "═══════════════════════════════════════════════\n"
            "GENRE-AWARE RULES (CRITICAL)\n"
            "═══════════════════════════════════════════════\n"
            "Before deciding, classify the study genre:\n"
            "A) Clinical outcome cohort/trial: Standard rules apply.\n"
            "B) PK/PD study: Usually 'No' unless clinical events were explicitly monitored.\n"
            "C) Assay validation / diagnostic accuracy: Usually 'No' unless clinical events were also tracked.\n"
            "D) Mechanistic / laboratory study: Almost always 'No'. Do NOT promote clinical events "
            "   (stroke, bleeding) mentioned in Introduction/Discussion as study outcomes when the study "
            "   itself is a laboratory or mechanistic investigation.\n"
            "E) Case report / case series: 'Yes' only if clinical events are explicitly described in the "
            "   case presentation (Methods/Results equivalent). HOWEVER, narrative hospital-course statements "
            "   such as 'no complications during hospitalization' or 'uneventful recovery' are NOT formal "
            "   outcome reporting and do NOT qualify as measured clinical outcomes.\n"
            "F) Guidance / review document: 'No' unless the paper reports original outcome data.\n\n"
            "═══════════════════════════════════════════════\n"
            "BASELINE-CONDITION SUPPRESSION (CRITICAL)\n"
            "═══════════════════════════════════════════════\n"
            "A disease state named in the title, inclusion criteria, or cohort description (the index "
            "condition) MUST NOT be counted as a clinical outcome unless a NEW event of that type is "
            "explicitly measured during follow-up.\n\n"
            "Examples:\n"
            "• Title: 'DOAC levels in acute ischemic stroke patients' - 'stroke' is the baseline population "
            "  definition, NOT an outcome, UNLESS the study also measures NEW stroke events during follow-up.\n"
            "• Inclusion: 'Patients with DVT on rivaroxaban' - DVT is the index condition, NOT an outcome, "
            "  UNLESS recurrent DVT is explicitly measured.\n"
            "• Cohort: 'Patients presenting with major bleeding' - the index bleeding event is NOT an outcome, "
            "  UNLESS NEW bleeding events during follow-up are measured.\n\n"
            "═══════════════════════════════════════════════\n"
            ...
            "• The index disease/condition that defines the study population\n"
            "• Narrative hospital-course statements without formal outcome measurement\n\n"
            "SET TO 'NO' IF:\n"
            "• Outcomes are mentioned only in Introduction/Background without measurement in THIS study\n"
            "• The study is mechanistic/laboratory and clinical events are only background context\n"
            "• Only narrative clinical course is described without formal outcome recording\n"
            ...
            "• clinical_outcomes, clinical_outcome_followup_flat, clinical_outcome_definition_flat → null\n"
            "If this field = 'Yes', at least one outcome-related field should be populated.\n\n"
            "Be conservative. Only set to 'Yes' if there is clear evidence that clinical events were "
            "actually planned in Methods AND reported in Results."
        ),
    )
